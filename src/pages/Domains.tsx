...
const DomainsPage = () => {
  ...
  const handleImportDomains = (newDomains: Array<{
    domain: string;
    url: string;
  }>) => {
  ...
  const handleSwapDomain = (domainData: {
    domain: string;
    provider: 'google' | 'microsoft';
  }) => {
    // Update the selected domain with new domain name (keeping the same provider)
    setDomains(prev => prev.map(domain => domain.id === selectedDomainForSwap?.id ? {
      ...domain,
      domain: domainData.domain
    } : domain));
    toast({
  ...
  const getProviderBadge = (provider: string) => {
    ...
    return <span className={`px-2 py-1 rounded text-xs font-medium ${isGoogle ? 'bg-blue-900/30 text-blue-400 border border-blue-400/30' : 'bg-orange-900/30 text-orange-400 border border-orange-400/30'}`}>
        {provider}
      </span>;
  };
  return <MainLayout title="Manage Domains">
      ...
      {availableDomainSlots === 0 && <Alert className="bg-red-900/30 border-red-400/30 text-red-400 mb-4">
        ...
        <Table>
          ...
          <TableBody>
            {domains.map(domain => <TableRow key={domain.id} className="hover:bg-mailr-lightgray/10 border-mailr-lightgray">
                ...
                <TableCell>
                  <span className={`px-2 py-1 rounded text-xs ${domain.status === 'Active' ? 'bg-green-900/30 text-green-400' : 'bg-yellow-900/30 text-yellow-400'}`}>
                    {domain.status}
                  </span>
                </TableCell>
                <TableCell>{getProviderBadge(domain.provider)}</TableCell>
                <TableCell>
                  <Button variant="ghost" size="sm" onClick={() => openSwapModal(domain)} className="h-8 w-8 p-0">
      ...
      <ImportDomainModal open={isImportModalOpen} onOpenChange={setIsImportModalOpen} onImport={handleImportDomains} />

      <SwapDomainModal open={isSwapModalOpen} onOpenChange={setIsSwapModalOpen} onSwap={handleSwapDomain} selectedDomain={selectedDomainForSwap} />
    </MainLayout>;
};
