...
const OrderGoogleInboxesPage = () => {
  ...
  const handleDisplayNameCountChange = (id: number, count: number) => {
    setInboxes(prevInboxes => 
      prevInboxes.map(inbox => 
        inbox.id === id ? { ...inbox, displayNameCount: count } : inbox
      )
    );
    toast.success(`Updated display name count to ${count}`);
  };
          ...
          <Table>
            <TableHeader className="bg-black/30">
              <TableRow className="hover:bg-transparent border-mailr-lightgray">
                ...
                <TableHead>Display names count</TableHead>
            ...
            <TableBody>
              {inboxes.map((inbox) => (
                <TableRow key={inbox.id} className="hover:bg-mailr-lightgray/10 border-mailr-lightgray">
                  ...
                  <TableCell>
                    <Select 
                      defaultValue={inbox.displayNameCount.toString()}
                      onValueChange={(value) => handleDisplayNameCountChange(inbox.id, parseInt(value))}
                    >
                      <SelectTrigger className="w-32 bg-mailr-darkgray border-mailr-lightgray">
